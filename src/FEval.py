...
class FEModel(ModelData):
    ...
    def makeModelCache(self):
        ...
        for k in self.Conn.keys():		# loop over elements
            ...
            midPoints.append( N.asarray(coord).mean(axis=0))
                #             midPoints.append( N.sum(N.asarray(coord))/ \
                #                               float(len(coord)) )

        self.elementMidPoints = N.asarray( midPoints )
        self.boundingbox = [mincoord, maxcoord]

        #         # find the corner nodes of the model
        #         # this is to find out about the size of the model
        #         if len(allcoord) > 1:
        #             allcoord = N.array( allcoord )
        #             minidx = N.argmin(allcoord, axis=0)
        #             maxidx = N.argmax(allcoord, axis=0)
        #             self.cornerNodes = []
        #             for idx in minidx:
        #                 self.cornerNodes.append( allcoord[idx] )
        #             for idx in maxidx:
        #                 self.cornerNodes.append( allcoord[idx] )
    ...
    def findClosestNode(self, coord, recalc=True):
        ...
        """
        if not self.allcoord or recalc:
...
#         return e
    ...
    def getVariables(self, element, nodvars=None, intpointvars=None):
        ...
        if nodvars:
            ...
            resNodvars = N.asarray(resNodvars)
    ...
    def findBoundaryNodes(self, recalc=False):
        ...
        """
        if not self.boundaryNodes or not recalc:
            ...
            for elename, sides in belems.items():
                ...
                for side in sides:
                    ...
                    for n in nodes:
                        bnodes.setdefault(n, set()).add(elename)
    ...
    def findModelBoundary(self, point, dir, accuracy=0.0001):
        ...
        point, dir = N.asarray(point), N.asarray(dir)
